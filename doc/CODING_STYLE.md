...
- ログにはツール名プレフィックスを付与する（`setup_logging(tool_name="ec-propagate")`）
- パイプラインのエラー追跡のため、どのツールが出力したログか一目で判別できるようにする
...
- NDJSONレコードを受け取るツールは、入口で `validate_record()` を呼んでフィールドの存在を検証する
- バリデーション関数は `lib/common/record.py` に定義されており、ARCHITECTURE.md の type フィールド定義と同期する
- バリデーションエラーは `RecordValidationError`（`ValueError` のサブクラス）として送出される
