...
- 実装より先にdoc/ARCHITECTURE.mdとの設計整合性を確認すること
...
- Unix哲学に従う（doc/ARCHITECTURE.md参照）
- クラス設計・責務分離はdoc/DESIGN.mdに従う
...
doc/CODING_STYLE.md を参照。言語共通のルールと言語別の規約を定義している。
...
1. **作業開始前**: 現在のブランチを確認し、作業対象のブランチをユーザに確認する。新規ブランチが必要な場合は命名をユーザと合意してから作成する
2. **作業中**: 設計変更を伴う場合はdoc/ARCHITECTURE.mdとの整合性を先に確認する
3. **作業完了後**: 変更内容をコミットする。コミットメッセージはユーザに提示せず、Git運用規約に従って作成する
4. **クリーンアップ**: サブエージェントが自動生成したドキュメント等の不要ファイルがルートや作業ディレクトリに残っていないか確認し、残っていれば削除する。プロジェクトに無関係なファイルをコミットしてはならない
